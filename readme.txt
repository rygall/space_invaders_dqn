...
testing.py will then run that episode's weights through the environment to determine the action sequence to play, and then subsequently play that action sequence. The first part has render mode turned off, so the game window will only display once the action sequence is running.s